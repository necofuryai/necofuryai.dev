...
| Edit config or add a binding | Installed `wrangler/config-schema.json` (usually under `node_modules`) — **absent in this repo**: wrangler is not a dependency and deploys run through Cloudflare's Git integration, so use the configuration reference instead; [configuration reference](https://developers.cloudflare.com/workers/wrangler/configuration/) |
