...
| Wrangler config schema | `node_modules/wrangler/config-schema.json` — **absent in this repo**: wrangler is not a dependency and deploys run through Cloudflare's Git integration, so fall back to the Wrangler docs above | Config fields, binding shapes, allowed values |
