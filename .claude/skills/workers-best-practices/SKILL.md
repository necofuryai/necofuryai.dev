...
| Wrangler config schema | `node_modules/wrangler/config-schema.json` — **absent in this repo**: wrangler is not a dependency and deploys run through Cloudflare's Git integration, so fall back to the Cloudflare docs below | Config fields, binding shapes, allowed values |
...
- `references/static-assets/` — Static Assets binding: configuration, `not_found_handling`, `html_handling`, routing precedence, gotchas
- `references/observability/` — Workers Logs / head sampling: configuration, query patterns, gotchas
