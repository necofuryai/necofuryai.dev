...
The three overrides for `@swup/plugin@<4`, `minimatch@<10.2.2`, and `brace-expansion@<5.0.8` are a coordinated set rather than independent version pins. Together they remove the dependency path to the vulnerable version of `brace-expansion`; removing any one of them breaks the build.
